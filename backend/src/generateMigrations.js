...
modelNames.forEach((modelName) => {
  try {
    ...
    if (rawAttributes && typeof rawAttributes === "object") {
      ...
      console.error(`Error: Model definition for ${modelName} is missing or invalid.`);
    }
  } catch (error) {
    console.error(`Error generating migration for ${modelName}:`, error.message);
    fs.appendFileSync("migration_errors.log", `${new Date().toISOString()} - ${modelName}: ${error.message}\n`);
  }
});
